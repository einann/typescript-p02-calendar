import { DayArray } from "../interfaces/CalendarFormatter";
...
export class Calendar {
    ...
    getDateObj() {
        ...
        if (firstDayInCalendar == 0) firstDayInCalendar = 1;
        let CURRENT_MONTH: boolean = firstDayInCalendar == 1 ? true : false;
        const dayArray: DayArray[] = [];

        let dm: number = month, dy: number = year;
        if (!CURRENT_MONTH) dm--;
